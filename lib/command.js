...
exports.apply = () => {
  if (fs.existsSync(SLACK_ARCHIVE_ARCH)) {
    return this.applyArch();
  }
...
exports.applyArch = () => {
  console.log(`Adding Tweaks Code to Slack (${ASAR_ARCH})... `);
  console.log('');

  // Make a backup
  fs.copyFileSync(SLACK_ARCHIVE_ARCH, `${SLACK_RESOURCES_DIR}/${ASAR_ARCH}.backup`);
  // Extract archive
  asar.extractAll(`${SLACK_ARCHIVE_ARCH}`, `${SLACK_RESOURCES_DIR}/${ASAR_ARCH}.unpacked`);
  // Add js code and recreate the package
  fs.appendFileSync(SLACK_FILE_PATH_ARCH + 'preload.bundle.js', generateJS());

  asar.createPackage(`${SLACK_RESOURCES_DIR}/${ASAR_ARCH}.unpacked`, `${SLACK_RESOURCES_DIR}/${ASAR_ARCH}`);
  console.log(chalk.green('Slack successfully tweaked. Please restart Slack to apply the tweaks.'));
};
