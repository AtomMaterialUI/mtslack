...
function getWindowsDirectory(customLocation) {
  ...
  for (let i = minorVersionMax; i >= 0; --i) {
    for (let j = patchVersionMax; j >= 0; --j) {
      ...
      if (customLocation) {
        const dir = `${customLocation}\\app-${ver}\\resources`;
...
function getMacDirectory(customLocation = 'Applications/Slack.app') {
  return `/${customLocation}/Contents/Resources`;
}

function getDirectory(customLocation) {
  switch (osType) {
    case 'Linux':
      return LINUX_DIR;
    case 'Windows_NT':
      return getWindowsDirectory(customLocation);
    case 'Darwin':
      return getMacDirectory(customLocation);
    default:
      return getMacDirectory(customLocation);
  }
}
...
const SLACK_RESOURCES_DIR = getDirectory(customLocation);
